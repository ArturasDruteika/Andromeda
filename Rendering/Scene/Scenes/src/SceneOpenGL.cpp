...
namespace Andromeda::Rendering
{
	...
	void SceneOpenGL::AddLuminousObject(int id, PhysicalProperties::LuminousBehavior* pLuminousObject)
	{
		m_pSceneOpenGLImpl->AddLuminousObject(id, pLuminousObject);
	}

	const std::unordered_map<int, const PhysicalProperties::DirectionalLight*> SceneOpenGL::GetDirectionalLights() const
	{
		return m_pSceneOpenGLImpl->GetDirectionalLights();
	}

	const std::unordered_map<int, const PhysicalProperties::PointLight*> SceneOpenGL::GetPointLights() const
	{
		return m_pSceneOpenGLImpl->GetPointLights();
	}

	const std::unordered_map<int, PhysicalProperties::LuminousBehavior*>& SceneOpenGL::GetLuminousObjects() const
	{
