...
namespace Andromeda::Rendering
{
	...
	void RenderableObjectOpenGL::CreateAndBindVertexBuffers(const std::vector<PhysicalProperties::Vertex>& vertices)
	{
		// Generate and bind VBO
		glCreateBuffers(1, &m_VBO);
		glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
		glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(PhysicalProperties::Vertex), vertices.data(), GL_STATIC_DRAW);
	}
