...
namespace Andromeda::Rendering
{
	...
	public:
		...
		void AddLuminousObject(int id, Space::LuminousBehavior* pLuminousObject) override;

		// === From ISceneLighting ===
		const std::unordered_map<int, const Space::DirectionalLight*> GetDirectionalLights() const override;
		const std::unordered_map<int, const Space::PointLight*> GetPointLights() const override;
		const std::unordered_map<int, Space::LuminousBehavior*>& GetLuminousObjects() const override;
